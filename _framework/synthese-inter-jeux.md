...
**Nuance que la vérification a ajoutée, et elle est importante** : aucun des huit originaux n'a d'état global unique. Ce qui tient lieu de machine à états est un ensemble de drapeaux et d'index indépendants — quatre chez Pokémon, posés par trois routines différentes. La moitié des glitches du corpus vient de là. Un `enum GameState` avec **un seul** champ courant et **un seul** point d'écriture, c'est ce que l'original ne pouvait pas se permettre et que rien n'empêche d'avoir.
