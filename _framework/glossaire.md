...
**FSM** (Finite State Machine, machine à états finis) — un système qui ne peut être que dans un seul état à la fois parmi un nombre fixe d'états, avec des règles explicites pour passer de l'un à l'autre. S'applique à deux échelles dans ce dépôt : la FSM globale (niveau 1 de chaque décorticage — Exploration/Combat/Menu) et une FSM locale plus petite (le pattern State pour le déroulement interne d'un combat). À noter : **Pokémon n'a pas de FSM globale au sens strict** — l'état y est porté par des drapeaux indépendants, et c'est la cause directe du glitch Trainer-Fly. Le cas n'a été vérifié que pour ce jeu ; c'est une structure courante à l'époque, mais le corpus ne l'établit pas pour les sept autres.
